...
//!
//! - publish a registry entry, so a second copy can see this run before starting its own
...
pub fn run(options: Options<'_>) -> std::io::Result<Outcome> {
	...
	// and written at the end, for the reason `local migrate` writes it first: two domains granted
...
mod tests {
	...
	/// held by another copy. The item is reported as somebody else's and the run continues.
	#[test]
