...
/// same rule: the table is paid for, `local tn` saves over it at the end of a scan, and reading a
...
mod tests {
	...
	fn a_recorded_suggestion_reaches_the_prompt() {
		// Writing the entry is the decision. `local tn` prints and only records when asked, so
	...
	fn a_broken_table_is_an_error_rather_than_an_empty_one() {
		// `local tn` saves the whole table back at the end of a scan, so reading a broken one as
