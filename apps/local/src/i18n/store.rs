...
/// parse must never be read as an empty one: every locale would report missing, `local i18n` would
...
mod tests {
	...
	fn a_broken_sidecar_is_an_error_rather_than_an_empty_one() {
		...
		// `local i18n` buys the article again, and the save at the end writes over the file a
