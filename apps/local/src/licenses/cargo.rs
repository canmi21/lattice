...
mod tests {
	...
	fn keeps_registry_crates_and_drops_the_workspace_own() {
		let metadata: Metadata = serde_json::from_value(serde_json::json!({
			"workspace_members": ["local 0.1.0 (path+file:///repo/apps/local)"],
			"packages": [
				{
					"id": "local 0.1.0 (path+file:///repo/apps/local)",
					"name": "local",
					"version": "0.1.0",
					"license": "MIT",
					"authors": [],
					"manifest_path": "/repo/apps/local/Cargo.toml",
					"source": null
				},
				{
		...
		assert_eq!(found[0].origins["local"], ["pkg:cargo/serde@1.0.219"]);
		assert_eq!(found[0].dependents.iter().collect::<Vec<_>>(), ["workspace:local"]);
