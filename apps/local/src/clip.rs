//! The `local clip` command: describing a video for someone who cannot watch it.
//!
//! Not a branch of `local alt`. That command hands a runner one file and asks it to look, which is
...
//! `alt::pending` already leaves clips out for exactly this reason. Same shape as `local diagram`:
...
//! under the source locale -- so `local locale` carries it into every language with no branch
...
pub async fn run(options: Options<'_>) -> std::io::Result<Outcome> {
	...
	loop {
		while running.len() < PARALLEL {
			...
			// Claimed before anything is spent, the same as `local alt`: sampling a clip is a minute
		...
		match result {
			Ok((text, spend, model, words)) => {
				...
				// Applied as it arrives and re-read inside the writer, for the reasons `local alt`
