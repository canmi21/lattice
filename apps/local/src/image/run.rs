//! The `local image` command: what the articles ask for, derived and published.
//!
...
pub fn run(
	...
	// in no position to make. `local migrate` does this for the corpus in one pass; this is the same
...
/// only derives pictures. A clip's rungs are asked about by `local video`, against `video/`.
...
/// rather than by the article. A picture rather than a record: only `local video` knows what a clip
...
///
/// Used by the migration and by `local alt`, both of which change a record without touching a
...
mod tests {
	...
	fn the_committed_manifest_is_refused_here_until_it_has_been_migrated() {
		...
		match load(&path) {
			...
			Err(error) => assert!(error.to_string().contains("local migrate"), "{error}"),
		}
	}
}
