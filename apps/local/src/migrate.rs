//! The `local migrate` command: filling in, once, what the records on disk cannot say themselves.
//!
...
//! onto its new key. Dry by default, the way `local gc` is: the listing is the review and `--live`
...
fn rewrites(
	...
	for image in &scan.images {
		...
		// the first is done and the second is what `local image` is for.
...
mod tests {
	...
	fn planning_writes_nothing() {
		// The listing is the review and `--live` is the answer to it, exactly as `local gc` does it.
		let (_temporary, root) = scenario();
