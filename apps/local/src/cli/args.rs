//! What `local` accepts, stated once as types.
//!
...
mod tests {
	...
	fn an_unknown_runner_names_the_choices() {
		let error = Cli::try_parse_from(["local", "alt", "--model", "gpt-5"])
	...
	fn a_repeated_option_collects_rather_than_replacing() {
		let cli = Cli::try_parse_from(["local", "i18n", "--locale", "zh-CN", "--locale", "ja-JP"])
			.expect("parse");
		match cli.command {
	...
	fn a_caption_import_has_to_say_what_language_it_is_in() {
		...
		assert!(Cli::try_parse_from(["local", "captions", "clip", "a.vtt"]).is_err());
	}
