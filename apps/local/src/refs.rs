...
/// ask the question, and `local gc` would have to be told about the second or sweep a clip away.
...
impl Scan {
	...
	/// migrated reference back to `local image` as a filename to look for and never find.
...
fn collect(file: &Path, text: &str, into: &mut Scan) {
	...
	// cropped directive would look unreferenced, and `local gc` would delete it.
