//! The `local og` command: one card per page per language, rendered here and published as-is.
//!
...
mod tests {
	...
	fn a_card_is_recorded_under_its_view_and_not_under_a_path() {
		// The key files the card in `local og`'s record; where its bytes land is decided by the
