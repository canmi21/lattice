//! The `local captions` command: a track cut to a clip already in the library, and attached to it.
//!
//! Separate from `local video` because the two arrive on different days -- a clip is cut on
...
pub fn run(
	...
	let record = merged
		...
		.ok_or_else(|| Error::NotAVideo(clip.to_owned()))?;
	if let Some(existing) = &replaced {
		// The bytes stay where they are. `local gc` is what sweeps an object no record points at,
