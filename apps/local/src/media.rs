...
//!
//! The manifest beside it holds variants, dimensions and a thumbhash -- all of which `local
...
//! the two apart is what stops `local image --force` from spending a rebuild's worth of pixels
...
/// The names `local tag` shows a model, and the reader for what it answers with -- see
...
pub struct Entry {
	...
	/// sits beside the description, under the same protection: `local image --force` rebuilds
