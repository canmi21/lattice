//! The `local summary` command: telling a reader what an article is about without telling them
//! what it concludes.
//!
...
fn pending(contents: &Path, force: bool) -> std::io::Result<(Vec<Article>, usize, usize)> {
	...
	while let Some(dir) = stack.pop() {
		...
		for entry in entries.flatten() {
			...
			// edited, so it would be bought and then be wrong. Unlike `local i18n` and `local tn`
