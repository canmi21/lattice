...
pub struct Article {
	...
	/// commands leave a draft alone and `local derived` does not count it as work outstanding, so
...
pub fn listing_at(repository: &Path) -> std::io::Result<Listing> {
	...
	for path in refs::markdown_under(&contents)? {
		...
		// The same test `local summary` and `local i18n` apply: no `lang`, no language to translate
