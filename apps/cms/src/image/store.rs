...
pub fn image_path(public_root: &Path, cid: &str, extension: &str) -> PathBuf {
	object_path(public_root, "image", cid, extension)
}

/// `video/{ab}/{cd}/{cid}.mp4` under the published root: one rung of a clip's ladder.
pub fn video_path(public_root: &Path, cid: &str, extension: &str) -> PathBuf {
	object_path(public_root, "video", cid, extension)
}

/// `captions/{ab}/{cd}/{cid}.vtt` under the published root: one text track.
pub fn caption_path(public_root: &Path, cid: &str, extension: &str) -> PathBuf {
	object_path(public_root, "captions", cid, extension)
}

/// The one scheme all three share, so a fourth kind is a prefix rather than a fourth function.
///
...
fn object_path(public_root: &Path, prefix: &str, cid: &str, extension: &str) -> PathBuf {
	...
	public_root.join(prefix).join(first).join(second).join(format!("{cid}.{extension}"))
}
...
/// separated by kind rather than by hash because a record describes a picture or a clip
/// indifferently, while `image/`, `video/` and `captions/` each hold one kind of bytes.
...
mod tests {
	...
	fn fans_a_variant_out_over_two_levels() {
		let path = image_path(Path::new("/pub"), "44b6081deaf0242ca3bf83d62a3b6c95", "avif");
	...
	fn nothing_in_a_key_says_what_an_object_belongs_to() {
		...
		let caption = caption_path(Path::new("/pub"), "abcdef0123456789abcdef0123456789", "vtt");
	...
	fn keeps_the_full_id_in_the_filename() {
		...
		let path = image_path(Path::new("/pub"), "abcdef0123456789abcdef0123456789", "webp");
	...
	fn survives_an_id_shorter_than_the_fanout() {
		...
		let path = image_path(Path::new("/pub"), "ab", "png");
