...
[styling.md](styling.md) under "The rail's box is one declared width"; what this file's rule reads
are `ONE_LINE` and `CLAMP` in [width.rs](../apps/cms/src/i18n/width.rs). The rail caps at 192px of
13px type there, so one line is 28 columns and the two-line clamp is 56. Read against the
