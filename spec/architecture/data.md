...
```
data/
  bucket/   the three bucket-shaped trees; two of them are mirrored
...
did anything derive it, did a tool make it. The draft tree is under `bucket/` because that is what
it is -- `wrangler dev` binds it as a bucket, and it is regenerated by `publish` like its two
neighbours. Grouping it with `record/` and `source/` would have filed an output with the inputs.

**Which two of the three are mirrored is the sync task's to say, not the directory tree's.** It
names both leaves and never their parent, so a tree added beside them is out of scope with no rule
written. That the draft tree now sits one level from being published is asserted rather than
assumed: `sync` refuses outright if a source it was given contains the draft tree, which is the
