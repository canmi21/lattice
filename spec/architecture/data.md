...
```
data/
  bucket/   the two bucket-shaped trees, both mirrored
...
compiles one into a root and the tree is gone. See [../drafts.md](../drafts.md).

**What is mirrored is the sync task's to say, not the directory tree's.** It names both leaves and
never their parent, so a tree added beside them is out of scope with no rule written -- asserted
rather than assumed: `sync` refuses outright if a source it was given is the directory holding the
mirrors rather than one of them.
...
rclone is pointed at `data/bucket/objects` and `data/bucket/metadata` by name and cannot see the
rest of `data/`. A directory added later is excluded because it was never in scope -- no rule to
...
rather than substituting: a named tone is still answered exactly or not at all, because a light
silhouette on a light surface is worse than a missing icon. The collector knows the site has one
icon; what selects between them knows only which files the record names.
