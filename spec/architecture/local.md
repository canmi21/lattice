# `local`, and the interface it answers for

`local` is the service that owns the collection: one implementation of every content operation,
with a command line over it and an HTTP surface beside that. It is the only writer of the authored
data -- see [../todo/milestones.md](../todo/milestones.md) for the boundary that makes that a rule
rather than a habit.

**The desktop window this file was written around is gone from here.** It ran as a Tauri shell with
a Milkdown editor inside it, and its source is kept to be read at
<https://github.com/canmi21/desktop-cms-archive>, where nothing builds. What survives it is in two
parts, and both are below: the shape of the service, which the window was merely a second shell of,
and what the window settled about the interface, which the next surface inherits. Filenames here
name files in that archive.

## The service is resident, and a shell is not

`local` stays running. It exists to do two things a command-line invocation structurally cannot:
**run the periodic work on a schedule**, and **answer an editor while somebody is writing**.

A schedule kept by a process that is only alive while a person is watching is not a schedule, and
an editor that has to be relaunched to save is not an editor. Both requirements land on the same
place: the layer below the shells has to own long-running work and its record, because an
invocation exits and takes its state with it.

The command line keeps its own reason to exist: builds and local workflows drive it, and a
scheduled task must remain runnable by hand with no editor open. Neither shell is the fallback for
the other.
...
Every capability has one in-process operation and two optional adapters. Either shell may expose
it, and neither owns the work. In particular, **a shell never spawns the other as a subprocess**:
doing so would turn terminal output and exit codes into an accidental internal API, duplicate
process lifecycle concerns, and make one surface depend on a separately discoverable binary.
Keeping the operation below both gives interactive actions, scripts and scheduled tasks the same
validation, effects and errors. The cost is an explicit library boundary and a small adapter in
...
entry rests in the quiet control's soft ink and lights under the pointer; a destructive one
rests the same and lights red instead of strong, and stays red once it has been asked for, so a menu
of ordinary actions does not read as a warning and the warning arrives with the choice.
...
**What an action offers has to be something that exists.** The row menu offered `local locale`
